...
export default function Rendering() {
  ...
  return <p style={{ color: 'red' }}>{error}</p>
      ...
      <p>Это нужно для корректного обновления элементов в DOM при изменениях. Ключ должен быть уникальным и стабильным (не индекс массива, если порядок может меняться). Math.random() также нельзя использовать для key</p>
...
function UserList() {
  return (
    <ul>
      ...
      <p>Если элементов немного и они не изменяются динамически, можно использовать индекс массива как <code>key</code>.
        Но в большинстве случаев лучше использовать значение или ID. Зачастую такие идентификаторы приходят с бэкенда и гарантируют уникальность,
        что делает работу с ключами (<code>key</code> в React) более надёжной и предсказуемой.</p>
